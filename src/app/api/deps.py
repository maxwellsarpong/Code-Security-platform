...
def get_tenant_from_api_key(
    ...
    return get_tenant_enforce_quota(x_api_key, session, current_user, enforce_quota=False)


def get_tenant_no_quota(
    x_api_key: Optional[str] = Header(None),
    session: Session = Depends(get_session),
    current_user: Optional[User] = Depends(get_current_user)
) -> Optional[Tenant]:
    """Alias for clear intent in routes that MUST bypass quota (like renewal)."""
    return get_tenant_from_api_key(x_api_key, session, current_user)


def get_tenant_enforce_scan_quota(
    x_api_key: Optional[str] = Header(None),
    session: Session = Depends(get_session),
    current_user: Optional[User] = Depends(get_current_user)
) -> Optional[Tenant]:
    """Auth + Rate limit + Monthly Scan Quota enforcement."""
    return get_tenant_enforce_quota(x_api_key, session, current_user, enforce_quota=True)


def get_tenant_enforce_quota(
    x_api_key: Optional[str] = Header(None),
    session: Session = Depends(get_session),
    current_user: Optional[User] = Depends(get_current_user),
    enforce_quota: bool = True
) -> Optional[Tenant]:
    """Resolve API key OR logged-in User to Tenant and enforce rate limits.
    """
